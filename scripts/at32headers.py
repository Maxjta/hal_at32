...
def main(hal_path, output):
    ...
    for entry in sorted(hal_path.iterdir()):
        if not entry.is_dir() or not entry.name.startswith("at32"):
            continue

        # obtain series API headers
        series_headers = entry / "drivers" / "inc"
        for header in series_headers.iterdir():
            m = re.match(r"^at32[a-z0-9]+_([a-z0-9]+)\.h$", header.name)
            if not m:
                m = re.match(r"^at32[a-z0-9]+_([a-z0-9]+)_([a-z0-9]+)\.h$", header.name)
            if not m:
                continue
            api = m.group(1)
    ...
    for api, all_series in apis.items():
        header_file = output / ("at32_" + api + ".h")
        with open(header_file, "w") as f:
            f.write("/*\n")
            f.write(" * NOTE: Autogenerated file using at32headers.py\n")
            f.write(" *\n")
